...
function formatToStandardTime(timeStr: string): string {
  ...
  return `${standardHours}:${formattedMinutes} ${ampm}`
}
...
function svgToPngDataUrl(svgString: string, width: number, height: number): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const scale = 4 // Crisp 4x scale
      canvas.width = width * scale
      canvas.height = height * scale
      const ctx = canvas.getContext("2d")
      if (ctx) {
        // Draw the entire loaded SVG image to fill the canvas bounds completely (no source rect cropping)
...
export async function exportPdf(
  ...
  const certificationText =
    "I hereby certify that the information above is a true and correct report of the hours worked, having been recorded daily at the actual times of starting and ending my shift from my remote workspace."

  // Pre-render Symph Logo to crisp PNG data URL
  let logoPngUrl = ""
  try {
    logoPngUrl = await svgToPngDataUrl(SYMPH_LOGO_SVG, 74, 37)
  } catch (e) {
    console.error("Failed to pre-render Symph logo", e)
  }
  ...
  function renderHeader(monthLabel: string) {
    // 1. Title section (pushed down by +18 pt for a beautiful 0.75 inch top margin)
    doc.setFont("helvetica", "bold")
    doc.setFontSize(16)
    doc.setTextColor(31, 41, 55) // slate-800
    doc.text(settings.systemTitle || "Internship Daily Time Record", 40, 54)

    // 2. Symph Logo on the right side of the title (colored skyblue)
    if (logoPngUrl) {
      try {
        const logoWidth = 56
        const logoHeight = 28
        const logoX = 555.28 - logoWidth
        const logoY = 32
        doc.addImage(logoPngUrl, "PNG", logoX, logoY, logoWidth, logoHeight)
      } catch (e) {
        console.error("Failed to add Symph logo to PDF", e)
      }
    }
    ...
    doc.text(`Monthly Report: ${monthLabel}`, 40, 74)

    // 3. Metadata section using a borderless autoTable
    autoTable(doc, {
      startY: 94,
      body: [
        [
        ...
        [
          ...
          "Intern Role:",
          profile.internRole || "Not set",
        ],
        [
          "School:",
          profile.school || "Not set",
          "Supervisor:",
          profile.supervisor || "Not set",
        ],
        [
          "Course / Year:",
          `${profile.course || "Not set"}, ${profile.yearLevel || "Not set"}`,
          "Supervisor Title:",
          profile.supervisorTitle || "Not set",
        ],
        [
          "Internship Period:",
          `${formatDate(profile.internshipStart)} - ${formatDate(profile.internshipEnd)}`,
          "Generated:",
          formatDate(formatDateInput(new Date())),
        ],
      ],
      theme: "plain",
      styles: {
      ...
      columnStyles: {
        0: { fontStyle: "bold", cellWidth: 85, textColor: [31, 41, 55] },
        1: { fontStyle: "normal", cellWidth: 215, textColor: [75, 85, 99] },
        2: { fontStyle: "bold", cellWidth: 85, textColor: [31, 41, 55] },
        3: { fontStyle: "normal", cellWidth: 130, textColor: [75, 85, 99] },
      },
      margin: { left: 40, right: 40 },
    })

    // 4. Render metrics cards (pushed down to leave a good amount of space)
    const metricsY = 178
    const cardHeight = 38
    const cardWidth = 121.32
    const gap = 10
    const leftX = 40

    const metricsData = [
      { label: "Required Hours", value: profile.requiredHours.toFixed(2) },
      { label: "Rendered Hours", value: totalRendered.toFixed(2) },
      { label: "Approved Hours", value: approvedHours.toFixed(2) },
      { label: "Remaining Hours", value: remaining.toFixed(2) },
    ]

    metricsData.forEach((metric, index) => {
      const x = leftX + index * (cardWidth + gap)

      // Card Background
      doc.setFillColor(248, 250, 252) // slate-50
      doc.rect(x, metricsY, cardWidth, cardHeight, "F")

      // Card Border
      doc.setDrawColor(226, 232, 240) // slate-200
      doc.setLineWidth(0.5)
      doc.rect(x, metricsY, cardWidth, cardHeight, "D")

      // Card Label
      doc.setFont("helvetica", "bold")
      doc.setFontSize(6.5)
      doc.setTextColor(100, 116, 139) // slate-500
      doc.text(metric.label.toUpperCase(), x + 6, metricsY + 12)

      // Card Value
      doc.setFont("helvetica", "bold")
      doc.setFontSize(11)
      doc.setTextColor(15, 23, 42) // slate-900
      doc.text(metric.value, x + 6, metricsY + 28)
    })
  }

  function renderCertificationBlock(startY: number) {
    const maxWidth = pageWidth - 80
    const textLines = doc.splitTextToSize(certificationText, maxWidth)
    let y = startY
    ...
    const lineWidth = 180
    doc.line(40, y, 40 + lineWidth, y)
    doc.text("Intern Name & Signature", 40, y + 14)

    doc.line(pageWidth - 40 - lineWidth, y, pageWidth - 40, y)
    doc.text("Supervisor Name & Signature", pageWidth - 40 - lineWidth, y + 14)

    return y + 26
  }

  const entriesByMonth = entries.reduce<Record<string, DtrEntry[]>>(
    (acc, entry) => {
  ...
  monthKeys.forEach((key, index) => {
    ...
    autoTable(doc, {
      ...
      body: entriesByMonth[key].map((entry) => [
        formatDate(entry.date),
        getDayName(entry.date),
        formatToStandardTime(entry.amIn),
        formatToStandardTime(entry.amOut),
        formatToStandardTime(entry.pmIn),
        formatToStandardTime(entry.pmOut),
    ...
    const finalY = (doc as { lastAutoTable?: { finalY?: number } }).lastAutoTable
      ?.finalY
    const startY = finalY ? Math.min(finalY + 20, pageHeight - 120) : 520
    renderCertificationBlock(startY)
  })
  doc.save(`${profile.studentId || "intern"}-internship-dtr.pdf`)
}
